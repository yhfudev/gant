        ...
        if (found) {
            ...
            case MESG_BURST_DATA_ID:
                ...
                // in case client wishes to ignore these events and capture the fake one
                {
                    ...
                    chan &= 0x1f;
                    if (dbg)
                        fprintf(stderr, "ch %x seq %d last %d\n", chan,
                                seq, last);
                    if (!burstbuf[chan]) {
                        if (seq != 0)
                        ...
                        if (lseq[chan] + 1 != seq) {
                            ...
                            if ((bused[chan] % BSIZE) == 0) {
                                burstbuf[chan] =
                                    realloc(burstbuf[chan],
                                            bused[chan] + BSIZE);
                                bzero(burstbuf[chan] + bused[chan], BSIZE);
                            }
                            memcpy(burstbuf[chan] + bused[chan],
                                   buf + i + 4, 8);
                            bused[chan] += 8;
                            if (dbg)
                                fprintf(stderr, "seq0 %d lseq %d\n", seq,
                                        lseq[chan]);
                            if (seq == 3)
                                lseq[chan] = 0;
                            else
            ...
            default:
                if (rfn) {
                    ...
                    if (dlen > MESG_DATA_SIZE) {
                        fprintf(stderr,
                                "cresponse buffer too small %d > %d\n",
                                dlen, MESG_DATA_SIZE);
                        for (j = 0; j < dlen; j++)
            ...
            if (event) {
                ...
                if (cfn) {
                    if (dlen > MESG_DATA_SIZE) {
                        fprintf(stderr,
                                "rresponse buffer too small %d > %d\n",
                                dlen, MESG_DATA_SIZE);
                        for (j = 0; j < dlen + 1; j++)
...
ANT_SendBurstTransferA(uchar chan, uchar * data, ushort numpkts)
{
    ...
    for (j = 0; j < numpkts; j++) {
        ...
        for (i = 0; i < 8; i++)
            buf[1 + i] =
                hexval(data[j * 16 + i * 2]) * 16 +
                hexval(data[j * 16 + i * 2 + 1]);
        usleep(20 * 1000);
...
void
ANT_AssignChannelEventFunction(uchar chan, CHANNEL_EVENT_FUNC rf,
                               uchar * rbuf)
{
    cfn = rf;
    cbufp = rbuf;
